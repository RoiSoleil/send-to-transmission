...
function init() {
    ...
    document.getElementById("notification-display").checked = JSON
	    .parse(localStorage.displayNotification);
    ...
    document.getElementById("refresh-rate").value = localStorage.refreshRate
	    || "";
    document.getElementById("enable-additional-paths").checked = JSON
	    .parse(localStorage.getItem("enable-additional-paths"));
    document.getElementById("use-metainfo").checked = JSON
	    .parse(localStorage.useMetainfo);
...
$(document)
	.ready(
		function() {
		    ...
		    $('.path-row:first').find('.path-label').keypress(
			    function() {
				if ($('.path-row:first')
					.find('.path-directory').val())
				    $('input#add-path').attr('disabled', '');
				else
				    $('input#add-path').attr('disabled',
					    'disabled');
			    });
		    $('.path-row:first').find('.path-directory').keyup(
			    function() {
				if ($('.path-row:first')
					.find('.path-directory').val())
				    $('input#add-path').attr('disabled', '');
...
<tr class="path-row">\
    ...
    <td><input type="button" value="x" /></td>\
					...
					new_label = parent_row.find(
						".path-label").val();
					new_dir = parent_row.find(
						".path-directory").val();
					...
					$(new_row).find(".path-label").val(
						new_label);
					$(new_row).find(".path-directory").val(
						new_dir);

					$(parent_row).find(".path-label").val(
						"");
					$(parent_row).find(".path-directory")
						.val("");
...
function save() {
    ...
    localStorage.setItem("displayNotification", document
	    .getElementById("notification-display").checked);
    localStorage.setItem("notificationDuration", document
	    .getElementById("notification-duration").value);
    localStorage.setItem("refreshRate",
	    document.getElementById("refresh-rate").value);
    localStorage.setItem("rpc_version", rpc_version);
    localStorage.setItem("enable-additional-paths", document
	    .getElementById("enable-additional-paths").checked);
    localStorage.setItem("useMetainfo",
	    document.getElementById("use-metainfo").checked);
...
function togglePaths() {
    var inputs = document.getElementById("paths-table").getElementsByTagName(
	    "input");
    var ths = document.getElementById("paths-table").getElementsByTagName("th");

    for (var i = 0; i < inputs.length; i++)
	inputs[i].disabled = !document
		.getElementById("enable-additional-paths").checked;
...
function toggleNotification() {
    var inputs = document.getElementById("notification-table")
...
port.onMessage
	.addListener(function(msg) {
		...
		case 200:
		    ...
		    document.getElementById("rpc-test-url").innerHTML = "Success! RPC version "
			    + rpc_version;
		...
		case 0:
		    ...
		    break;
		default:
		    document.getElementById("rpc-test-url").innerHTML = "Error: "
			    + msg.req.status;
		    document.getElementById("rpc-test-url").className = "result error";
		    break;
		}
